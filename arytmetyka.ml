...
let odwrotny x =
  ...
  | Niespojny (a, b) ->
    if( a > 0. && b < 0.)
      then porzadkuj ( Niespojny (1. /. a, 1. /. b) )
...
let mnozenie_spojnych x y =
  ...
  | Spojny(a, b), Spojny(c, d) ->
    if (modul a = 0. && modul b = 0. ) || (modul c = 0. && modul d = 0.)
      then Spojny ( 0. , 0.)
    else
      let lewy =
        min
          (min (mnoz_ignoruj_nan a c) (mnoz_ignoruj_nan a d ) )
          (min (mnoz_ignoruj_nan b c)(mnoz_ignoruj_nan b d) )
      and prawy =
        max
          (max(mnoz_ignoruj_nan a c) (mnoz_ignoruj_nan a d) )
          (max( mnoz_ignoruj_nan b c ) ( mnoz_ignoruj_nan b d))
